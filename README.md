# <a name="documentation"></a>Human enterovirus genome annotation

## [How to annotate EV sequences with VADR](#howto)

## [EV VADR model libraries](#evmodels)
...
---
## <a name="howto"></a>How to annotate EV genomes with VADR

Steps for using VADR for EV annotation:

1. Update: requires new version of VADR.
 
2. Clone the latest EV VADR model library from this repository. <!-- (current release v1.0)<br/>-->
   `git clone git@github.com:greninger-lab/vadr-models-ev.git`<br/>
   <!--or download the current release from [here](https://github.com/greninger-lab/vadr-models-hpiv/releases/tag/v1.0).</br>-->
   Note the path to the directory name created plus the "evA", "evB", "evC", or "evD"
   subdirectory (e.g. /path/to/vadr-models-ev/evA) as `<ev-models-dir-path>`
   for step 4.
...
4. Run the `v-annotate.pl` program on an input trimmed fasta file with
   ...
   For evB, run:
```
v-annotate.pl -r --r_file <ev-models-dir-path>/evB.rpn.fa --mkey <ev-key> --mdir <ev-models-dir-path> <fasta-file-to-annotate> <output-directory-to-create>
```
...
* This page was adapted for EV from [Mpox virus annotation](https://github.com/ncbi/vadr/wiki/Mpox-virus-annotation)
